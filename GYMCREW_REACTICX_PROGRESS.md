...
**A real debugging detour worth recording:** the verification screenshot run threw a `React error #418` (hydration mismatch) — investigated it properly rather than dismiss or paper over it. Built four isolated test routes (`Button.Root`+`containerStyle` alone, `EditableText` alone, `WelcomeWidget`'s full structure minus the button, and finally the exact *original pre-migration* `WelcomeWidget` code) and found the error reproduces identically with the unmodified original code in the same harness — it's an artifact of this session's `_dev-*`-prefixed temporary route naming (Expo Router likely special-cases underscore-prefixed routes), not a bug introduced by the migration. Confirmed the app's real routes (`sign-in.html`, `onboarding/welcome.html`) stayed clean throughout. All five temporary isolation routes deleted before committing. Lesson recorded for future verification passes: don't prefix temporary preview routes with `_`.

**Remaining work:** `GoalsWidget`, `AnnouncementBanner`, `VisualTrainingCalendar` untouched this pass. Real device/authenticated-session verification of the whole Home screen (with real store data) is still outstanding — this is why the checklist item stays `[~]`, not `[x]`.
