    ...
    const propagateProps = (children: React.ReactNode): React.ReactNode => {
      return React.Children.map(children, (child) => {
        ...
        if (child.type === Shimmer || child.type === ShimmerEffect) {
          const childProps = child.props as IShimmerEffect;
          return React.cloneElement(child as React.ReactElement<any>, {
            isLoading,
            preset: childProps.preset || preset,
            shimmerColors: childProps.shimmerColors || shimmerColors,
            duration: childProps.duration || duration,
            direction: childProps.direction || direction,
            variant: childProps.variant || variant,
            opacity: childProps.opacity ?? opacity,
          });
        }
        ...
        if (childProps && childProps.children) {
          return React.cloneElement(child as React.ReactElement<any>, {
            children: propagateProps(childProps.children),
          });
        }
