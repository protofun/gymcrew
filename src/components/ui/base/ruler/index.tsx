...
// GymCrew patch: `initialValue` prop (the ruler used to always start at minValue), and `onValueChange` now only
// fires while the user is dragging — it used to fire during the mount animation as well, which made the
// value jump around before anyone touched it.
// @ts-check
...
export const Ruler: React.FC<IRuler> & React.FunctionComponent<IRuler> =
  memo<IRuler>(
    ({
      ...
      const startX = -((initialValue ?? minValue) - minValue) * step;
      const translateX = useSharedValue<number>(animateOnMount ? -width : startX);
      ...
      useEffect(() => {
        if (animateOnMount) {
          translateX.value = withSpring<number>(startX, SPRING_CONFIG_SOFT);
        }
