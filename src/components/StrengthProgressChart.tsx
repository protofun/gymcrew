...
export function StrengthProgressChart({ exerciseName, points, title = "Strength Progress", unit = "kg" }: StrengthProgressChartProps) {
  ...
  function handleTouch(locationX: number) {
    ...
    const stepX = CHART_WIDTH / (count - 1);
    const index = Math.round(chartX / stepX);
  ...
  const coords = points.map((point, index) => ({
    x: index * stepX,
    y: CHART_HEIGHT - CHART_PADDING - ((point.value - min) / range) * (CHART_HEIGHT - CHART_PADDING * 2),
  }));
    ...
    <View className="gap-3">
      ...
      >
        <Svg width="100%" height={CHART_HEIGHT} viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`}>
          ...
          {active && (
            <>
              <Line x1={active.coord.x} y1={0} x2={active.coord.x} y2={CHART_HEIGHT} stroke={colors.neutral.textSecondary} strokeWidth={1} strokeDasharray="3,4" />
      ...
      <View className="flex-row justify-between">
        <Text className="caption text-text-secondary">{formatDateShort(points[0].date)}</Text>
        <Text className="caption text-text-secondary">{formatDateShort(points[points.length - 1].date)}</Text>
      </View>
    </View>
  );
}
